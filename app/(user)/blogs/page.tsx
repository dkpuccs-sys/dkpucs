      ...
      <div className="space-y-8">
        {blogs.length > 0 ? (
          blogs.map((blog: any) => {
            ...
            return (
              <article key={blog.id} className="p-6 border rounded-lg shadow-md bg-card">
                ...
                <p className="text-foreground mb-3 whitespace-pre-wrap">{snippet}</p>
                <Link href={`/blogs/${blog.id}`} className="text-sm text-primary hover:underline font-medium">
                  Read full blog
                </Link>
              </article>
            )
      ...
      {totalBlogs > ITEMS_PER_PAGE && (
        <div className="flex justify-center mt-8 space-x-2">
          <Button asChild variant="outline" disabled={currentPage === 1}>
            <Link href={`/blogs?page=${currentPage - 1}${levelFilter ? `&level=${encodeURIComponent(levelFilter)}` : ''}`}>Previous</Link>
          </Button>
          {Array.from({ length: totalPages }, (_, i) => (
            <Button key={i + 1} asChild variant={currentPage === i + 1 ? "default" : "outline"}>
              <Link href={`/blogs?page=${i + 1}${levelFilter ? `&level=${encodeURIComponent(levelFilter)}` : ''}`}>{i + 1}</Link>
            </Button>
          ))}
          <Button asChild variant="outline" disabled={currentPage === totalPages}>
            <Link href={`/blogs?page=${currentPage + 1}${levelFilter ? `&level=${encodeURIComponent(levelFilter)}` : ''}`}>Next</Link>
          </Button>
        </div>
      )}
    </div>
