...
export default function TextbookCard({ textbook }: TextbookCardProps) {
  ...
  return (
    <>
      <Card key={textbook.id} className="border shadow-sm hover:shadow-md transition-shadow">
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <CardTitle className="mb-2 text-lg sm:text-xl">{textbook.title}</CardTitle>
              <CardDescription className="flex items-center gap-4 flex-wrap text-xs sm:text-sm">
                <span className="text-muted-foreground">
                  {textbook.author && `by ${textbook.author} | `}
                  {textbook.subject} ({textbook.section}) | {new Date(textbook.createdAt).toLocaleDateString()}
                </span>
              </CardDescription>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="border-t border-border pt-4">
              <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed mb-4">
                Hyperlink:{" "}
                <a href={textbook.hyperlink} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                  {textbook.hyperlink}
                </a>
              </p>
            </div>
